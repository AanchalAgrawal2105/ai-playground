...
class SlackReportFormatter:
    ...
    def create_health_report_blocks(
        ...
        health_emoji = "🟢" if overall_health >= 95 else "🟡" if overall_health >= 85 else "🔴"
        blocks.append({
            "type": "header",
            "text": {
                "type": "plain_text",
        ...
        if failures:
            ...
            if failures:
                critical_failure = max(failures, key=lambda x: x.get('duration_hours', 0))
                blocks.append({
                    "type": "section",
                    "text": {
                        "type": "mrkdwn",
        ...
        if consistently_failing_dags and len(consistently_failing_dags) > 0:
            ...
            for idx, dag_info in enumerate(consistently_failing_dags[:10], 1):  # Show top 10
                ...
                consecutive_failures = dag_info.get('consecutive_failures', failure_count)
    ...
    def _generate_pattern_analysis(anomalies: List[Dict[str, Any]]) -> str:
        ...
        common_prefix = max(set(dag_prefixes), key=dag_prefixes.count) if dag_prefixes else None

        if common_prefix:
            count = dag_prefixes.count(common_prefix)
            patterns.append(f"• `{common_prefix}` pipelines showing systematic issues ({count} affected)")

        # Check for resource patterns
        resource_issues = [a for a in anomalies if 'resource' in str(a.get('root_cause', '')).lower()]
        if resource_issues:
            patterns.append(f"• Resource contention detected in {len(resource_issues)} pipelines")

        return "\n".join(patterns) if patterns else "• Multiple independent issues detected"
...
if __name__ == "__main__":
    ...
    recommendations = [
        {'priority': 'URGENT', 'action': 'Investigate EMR EKS cluster resource allocation for pricing pipelines'},
        {'priority': 'HIGH', 'action': 'Check ops-dap-bi-enriched infrastructure dependencies'},
        {'priority': 'MEDIUM', 'action': 'Review loyalty pipeline data sources and connections'},
    ]
